...
class Template {
   ...
   * @return string
   */
  public function parse($template_filename) {
    // If a basedir is defined,
    // append it to the front of the filename
    if ($this->basedir) {
      $template_filename = $this->basedir . $template_filename;
    }

    // If the template file isn't already in the cache, fetch it
    if (!isset($this->cache[$template_filename])) {
      ...
      $this->cache[$template_filename] =
        "?" . ">\n" .
        implode("",file($template_filename)) .
        "<" . "?php\n" ;
    }
    ...
    eval($this->cache[$template_filename]);
